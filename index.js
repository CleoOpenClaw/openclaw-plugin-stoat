...
function accountFromConfig(cfg, accountId = DEFAULT_ACCOUNT_ID) {
  ...
  return {
    ...
    token,
    email: accountCfg.email ?? channelCfg.email ?? "",
    password: accountCfg.password ?? channelCfg.password ?? "",
...
async function loginToGetToken(account) {
  const email = account.email;
  const password = account.password;
  if (!email || !password) return null;
  const res = await fetch(`${account.apiBase}/auth/session/login`, {
    method: "POST",
...
async function dispatchInbound(ctx, account, payload, selfId, options = {}) {
  ...
  ctx.log?.info?.(`[stoat] route resolved: agentId=${route.agentId} sessionKey=${route.sessionKey} isDirect=${isDirect}`);
  ...
  const { dispatcher, replyOptions, markDispatchIdle } =
    core.channel.reply.createReplyDispatcherWithTyping({
      ...
      deliver: async (replyPayload) => {
        ...
        // Always get a fresh token for send — session tokens expire quickly
  ...
  try {
    if (core.channel?.session?.recordInboundSession) {
      try {
        ...
        const resolvedStorePath = core.config.loadConfig()?.session?.store?.path
        ...
        await core.channel.session.recordInboundSession({
          ...
          onRecordError: (err) => {
            logger?.warn?.(`stoat recordInboundSession failed for ${inbound.chatId}: ${String(err)}`);
          },
        });
      } catch (err) {
        logger?.warn?.(`stoat recordInboundSession error for ${inbound.chatId}: ${String(err)}`);
      }
    }
    ...
    ctx.log?.info?.(`[stoat] dispatchReplyFromConfig completed for ${inbound.chatId}`);
  } finally {
    markDispatchIdle?.();
  }
}
...
export const stoatPlugin = {
  ...
  config: {
    ...
    isConfigured: (account) => Boolean(account.token || (account.email && account.password)),
    describeAccount: (account) => ({
      accountId: account.accountId,
      name: account.name,
      enabled: account.enabled,
      configured: Boolean(account.token || (account.email && account.password)),
      tokenSource: account.token ? account.tokenSource : (account.email && account.password) ? "email/password" : account.tokenSource,
      apiBase: account.apiBase,
    ...
    startAccount: async (ctx) => {
      ...
      if (!account.token) {
        ...
        if (freshToken) {
          account.token = freshToken;
          activeTokens.set(account.accountId, freshToken);
          ctx.log?.info?.(`[${account.accountId}] Auto-logged in to get fresh session token`);
        } else {
          throw new Error(
      ...
      const selfId = await fetchSelfUserId(account).catch(() => null);
      // If token is stale/expired, try re-login
      const resolvedSelfId = selfId ?? await (async () => {
        const freshToken = await loginToGetToken(account);
        if (freshToken) {
          account.token = freshToken;
          activeTokens.set(account.accountId, freshToken);
          ctx.log?.info?.(`[${account.accountId}] Refreshed stale session token`);
          return await fetchSelfUserId(account).catch(() => null);
        }
        return null;
      })();
      if (!resolvedSelfId) {
        ...
        ctx.log?.info?.(`[${account.accountId}] Stoat bot ID: ${resolvedSelfId}`);
      }
      ...
      const wsClient = createWsClient(ctx, account, resolvedSelfId);
      activeClients.set(account.accountId, { account, wsClient, selfId: resolvedSelfId });
